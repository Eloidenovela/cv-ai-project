...
async def authenticate_user(username: str, password: str):
    try:
        ...
        user = user_service.is_user({"username": username, "password": password})
...
async def create_access_token(data: dict, expires_delta: timedelta = None):
    to_encode = data.copy()
    if expires_delta:
        expire = datetime.now() + timedelta(minutes=30) #expires_delta  # Define o tempo de expiracao do token
    else:
